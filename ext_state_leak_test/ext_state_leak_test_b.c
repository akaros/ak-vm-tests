...
// TODO: STX values are tricky because instructions that treat them
//       as floats treat that set of registers as a stack, making it
//       painful to inspect (you have to pop all of the values to
//       read them, and then push them all back on so it looks like
//       nothing changed). We use FLD to push m80fp type values onto
//       the fpu register stack so that we are sure to touch all the
//       bits in the floating point registers.
         // NOTE: This might adjust our last-instruction opcode and
         //       thus affect our checks for the fp_header.
...
// We could probably check all these by using fldenv and fstenv, which
// save and restore the fp header information, though Intel is not
// very clear on what format these instructions end up conforming to on
// 64-bit processors. It's probably IA32e but....

uint16_t fpu_control_word = 0x1f00;
...
// In this half of the test we just use a goofy looking lattice so that
// any interference clearly shows up in the failure hexdump.
char *ymm0  = "/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\";
char *ymm1  = "\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/";
char *ymm2  = "/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\";
char *ymm3  = "\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/\\/";
...
void fail(char *s, char *cur, char *src, size_t size) {
  printf("Test B failure! Failed at state component: %s\n", s);
  printf("cur val:\n"); hexdump(cur, size);
  printf("src val:\n"); hexdump(src, size);
  exit(0);
}
...
void set_values(uint64_t xcr0) {
  ...
  // NOTE: This half of the fp_test should probably set custom values in the fpu, the other half
  //       just uses fninit. For now both actually just use fninit, however.
...
// TODO: My workstation doesn't have the below state components,
  ...
  //     (e.g. we can't hexdump what we don't have)
...
void check_values(uint64_t xcr0) {
  ...
  // TODO: The header is a pain, you have to check around reserved bits, etc.
  //       so we're skipping the header for now. :)
...
int main() {
  ...
  printf("Starting ext_state_leak_test_b.\n");
